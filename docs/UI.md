...
| High | Domain row density | Domain rows can overwhelm the admin with registrar, registered date, last check, and provider detail. | Keep the visible row to domain, status, and expiry. Put registrar, last check, provider, registry status, and name servers in a hover/focus tooltip. |
