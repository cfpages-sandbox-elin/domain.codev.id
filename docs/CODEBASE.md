...
| `src/utils/whoisSchedule.ts` | Frontend WHOIS schedule visibility helper. Mirrors the automatic check cadence closely enough for UI tracking: skips terminal available/reserved rows, schedules missing expiry retries, ramps owned domains from monthly/weekly/3-day/daily checks near expiry, and ramps target domains through expiry/drop-window checks. |
