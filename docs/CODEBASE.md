...
| `src/components/DomainItem.tsx` | Domain row. Shows Mine/To Snatch row background tints with status overrides for available/expired/unknown, clickable whois.com domain links, normalized long dates, basic expiry/status info including missing-expiry warning, persisted WHOIS details in tooltip, inline registrar select plus icon-only buy action, simple colored Mine/To Snatch icons, recheck/delete actions, and prevents available domains from being toggled to Mine in the UI. |
