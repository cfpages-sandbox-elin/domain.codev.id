...
| `src/App.tsx` | Main state machine. Handles session, domain list, transient WHOIS detail cache, provider dashboard state, notifications, logs, modals, 6-worker bulk processing, add/remove/toggle/recheck/export flows, and estimated drop timeline modal. Sets explicit light/dark app backgrounds and a darker dark-mode dashboard panel for stronger row contrast. |
...
| `src/components/DomainItem.tsx` | Domain row. Shows stronger Mine/To Snatch row backgrounds, a leading Mine/To Snatch icon before the domain name, status overrides for available/expired/unknown, clickable whois.com domain links, normalized long dates, basic expiry/status info including missing-expiry warning, persisted WHOIS details in tooltip, inline registrar select plus icon-only buy action, simple colored Mine/To Snatch action icons, recheck/delete actions, and prevents available domains from being toggled to Mine in the UI. |
| `src/components/DomainList.tsx` | List controls and rendering. Supports icon-labeled filters, Expiring Soon as the next 90 days sorted by nearest expiry, sort dropdown, re-check menu for all visible or missing-data domains, import/export menus, empty states, To Snatch filtering for available domains, and maps domains to `DomainItem`. |
