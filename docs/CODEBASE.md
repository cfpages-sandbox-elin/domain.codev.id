...
| `src/types.ts` | Core domain, domain tag (`mine`, `to-snatch`, `others`), WHOIS result, quota, provider attempt, provider status, provider credential input, Auto Mine rule, category manual override, category word group, user app settings, and integration client/scope types. |
...
| File | Purpose / logic |
| --- | --- |
| `src/components/AutoMinePanel.tsx` | Auto Mine rules panel. Edits app-level synced name-server combination rules, requires at least two unique name servers per rule, normalizes server names, matches only non-available domains whose stored `name_servers` contain every server in a rule, auto-applies matches by asking `App` to update matching rows to `mine`, provides manual apply, enable/disable, remove, and matched-domain preview controls. |
