...
| `src/components/floatingActionStyles.ts` | Shared fixed 48px circle geometry, 20px icon size, focus treatment, and primary/neutral tone variants for Add Domain and System Status actions. |
...
| `src/components/Tooltip.tsx` | Shared instant portal tooltip with viewport clamping/top flip. Tracks only the active tooltip instead of broadcasting hover state to every mounted tooltip, avoids duplicate pre-render position work, and closes on mouse leave, blur, Escape, page hide, and tab/window inactivity. |
