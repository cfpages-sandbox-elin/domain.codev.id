...
| `src/components/DomainList.tsx` | List state and rendering coordinator. Owns persisted filter/sort/category/TLD state, keyword filter state, hide-registered-target state, recheck/export menu state, category grouping, incremental rendering, row mapping, and calls focused `domain-list` helper modules for pure filter/sort/storage logic, filter chips, keyword search UI, and empty/loading/no-match states. |
