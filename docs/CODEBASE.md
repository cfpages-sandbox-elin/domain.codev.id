...
| `src/components/DomainItem.tsx` | Domain row. Shows Mine/To Snatch row background tints with status overrides for available/expired/unknown, clickable whois.com domain links, normalized long dates, basic expiry/status info, persisted WHOIS details in tooltip, inline registrar select plus icon-only buy action, simple colored Mine/To Snatch icons, recheck/delete actions, and prevents available domains from being toggled to Mine in the UI. |
| `src/components/DomainList.tsx` | List controls and rendering. Supports icon-labeled filters, Expiring Soon as the next 90 days sorted by nearest expiry, sort dropdown, visible-list recheck, import/export menus, empty states, To Snatch filtering for available domains, and maps domains to `DomainItem`. |
