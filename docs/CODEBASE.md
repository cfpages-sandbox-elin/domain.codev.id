...
| `src/components/app/DashboardView.tsx` | Dashboard view shell. Owns the responsive page chrome around `DomainList`, using lighter edge-to-edge mobile spacing and framed card styling from small screens upward, and passes through list state/actions from `App`. |
...
| `src/components/bulk-add/TagChoice.tsx` | Reusable responsive Mine/To Snatch/Others radio-card control and tag icon helpers used by `BulkAddModal`. |
...
| `src/components/StatusLog.tsx` | Floating viewport-constrained collapsible status log. Infers icon/color from emoji markers in log strings. |
