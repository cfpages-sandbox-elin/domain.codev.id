import type { RankMatchMode, SerpOrganicHit } from './serp-types.ts';
...
/** Best-effort registrable domain: last two labels, with common multi-part TLD awareness for .co.id / .com.au style. */
...
export const findBestRankHit = (
  ...
  for (const hit of organic) {
    ...
    if (matchMode === 'domain') {
      ...
      matches = Boolean(
        hitHost
        && (hitHost === tracked || hitHost.endsWith(`.${tracked}`)),
      );
